...
1. Проверьте, синхронизировалась ли информация о маршрутах (в столбце proxy_cidrs отображаются проксируемые подсети):
...
![Маршрутная информация](/assets/image-3.png)

2. Проверьте доступность узлов в проксируемой подсети:
...
1. Запустите EasyTier с включенным порталом WireGuard:
...
2. Получите конфигурацию для WireGuard-клиента:
...
Вы можете запустить собственный публичный общий узел, чтобы помогать другим узлам обнаруживать друг друга. [Публичный общий узел](https://info.qtet.cn/uptime/easytier) — это обычная сеть EasyTier (с теми же именем и ключом), к которой могут подключаться другие сети.
