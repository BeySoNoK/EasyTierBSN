...
Однокомандная регистрация в качестве системной службы (автоматический запуск в фоновом режиме при загрузке системы)
...
При использовании общего узла каждый подключающийся узел должен указывать одинаковые параметры --network-name и --network-secret для идентификации сети.
...
EasyTier по своей сути децентрализован — нет разделения на серверы и клиенты. Любое устройство может присоединиться к виртуальной сети, если оно может связаться хотя бы с одним узлом в ней. Ниже показано, как настроить децентрализованную сеть:

Запустите первый узел (узел A):
